...
- [x] **결과 영속화 근본 리팩터** (2026-04-15) — 증상: 어제 생성·풀이 완료한 세션을 오늘 또는 다른 기기에서 열면 `/result/{id}`가 영구 404. 근본 원인: (A) 결과가 오직 `question_bank`(DATABASE_URL 풀, 별개 Postgres)에만 저장되고 `user_sessions`에는 `result_json` 컬럼 자체가 없음 (B) Redis 2h TTL 만료 후 fallback이 question_bank에 의존하는데 `save_to_bank` 실패가 warning으로 삼켜져 고아 ready 세션 양산 (C) `_finalize_ready`가 DB 커밋 이전에 호출되는 순서 버그. 수정: (1) migration 004로 `user_sessions`에 `result_json/error_message/completed_at` 컬럼 추가 및 `(user_id,status)` 인덱스 (2) `user_store.finalize_session`이 result_json + status='ready' + completed_at을 원자 UPDATE로 커밋 (3) `_run_generation` 순서 반전: user_sessions primary write 성공 후에만 memory complete, question_bank는 best-effort (4) `_load_result_from_db` 우선순위 재정의: memory → `user_sessions.result_json` → 레거시 question_bank + backfill → 404, 실패 사유(`user_sessions_row_missing`/`legacy_no_pdf_hash`/`question_bank_miss`/`user_sessions_query_error`)를 `logger.error`로 명시 (5) `save_to_bank` 반환 타입 `None→bool`, 실패 로그 warning→error (6) 복구 스크립트 2종: `scripts/count_orphan_sessions.py`(dry-run 카운트), `scripts/heal_orphan_sessions.py`(GROUP A backfill / GROUP B failed 전이). 테스트 `tests/unit/test_result_retrieval.py` 10개 추가. 상세: `documents/problem/2026-04-15-result-404-cross-device.md`, `documents/record_progress/2026-04-15-03-result-영속화-리팩터.md`
