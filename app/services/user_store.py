...
class UserStore:
    ...
    async def finalize_session(
        ...
        """생성 완료된 결과를 user_sessions에 원자적으로 커밋한다.

        result_json, status='ready', completed_at=now(), error_message=NULL을 단일
        UPDATE로 기록한다. 성공 시 True. 실패하거나 대상 행이 없으면 False.

        이 호출이 성공해야만 생성이 "완료" 상태로 간주된다. Redis/메모리 store는
        이후의 hot cache일 뿐이다.
        """
        pool = self._require_pool()
        async with pool.acquire() as conn:
            result = await conn.execute(
                ...
                "    error_message = NULL "
                "WHERE id = $2::uuid AND user_id = $3::uuid",
                result_json, session_id, user_id,
            )
        return result.endswith(" 1")
    ...
    async def mark_session_failed(
        ...
        """세션을 failed로 전이. error_message를 함께 기록."""
        pool = self._require_pool()
        async with pool.acquire() as conn:
            result = await conn.execute(
                "UPDATE user_sessions "
    ...
    async def get_session_with_result(
        self,
        user_id: str,
        session_id: str,
    ) -> Optional[dict]:
        """단일 세션 행 + result_json을 한 번에 조회.

        반환: {id, pdf_hash, status, result_json, error_message} 또는 None.
        result_json은 NULL일 수 있다(레거시 고아 세션).
        """
        pool = self._require_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT id::text, pdf_hash, status, result_json, error_message "
                "FROM user_sessions "
    ...
    async def backfill_result(
        self,
        user_id: str,
        session_id: str,
        result_json: str,
    ) -> bool:
        """레거시 경로: question_bank에서 복구한 결과를 user_sessions에 채워 넣는다.

        status가 이미 ready인 행에만 적용. 새로 만드는 것이 아니라 누락 컬럼을
        메우는 용도.
        """
        pool = self._require_pool()
        async with pool.acquire() as conn:
